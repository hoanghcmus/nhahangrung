...
public partial class Vn_Vn_Control_Menu : System.Web.UI.UserControl
{
    ...
    {
        List<TheLoai> listTheLoai = TheLoai.TheLoai_GetByCategoryAndParentIDAndWebsite("2", id, 3);
        string activeMenuID = Request.QueryString["MenuItemID"] ?? "1";
        ...
        if (listTheLoai != null && listTheLoai.Count() != 0)
        {
            ...
            foreach (TheLoai theLoai in listTheLoai)
            {
                ...
                {
                    sb.Append("<a href='" + theLoai.DuongDan_Vn + "'> <i class='li-icon'></i> <span>" + theLoai.TieuDe_Vn + "</span></a>");
                }
                else if (Session["lang"].ToString().Equals("en"))
                {
                    sb.Append("<a href='" + theLoai.DuongDan_En + "'> <i class='li-icon'></i> <span> <span>" + theLoai.TieuDe_En + "</span></a>");
                }
                else if (Session["lang"].ToString().Equals("ru"))
                {
                    sb.Append("<a href='" + theLoai.DuongDan_Ru + "'> <i class='li-icon'></i> <span> <span>" + theLoai.TieuDe_Ru + "</span></a>");
                }
                else if (Session["lang"].ToString().Equals("cn"))
                {
                    sb.Append("<a href='" + theLoai.DuongDan_Cn + "'> <i class='li-icon'></i> <span> <span>" + theLoai.TieuDe_Cn + "</span></a>");
                }
